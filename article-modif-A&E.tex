...
The estimations are computed using the following expressions {\color{red} plus subtil qu'un calcul empririque ? si oui, expliciter}:
\begin{equation}
...
%\begin{equation}
%  \label{eq:cheby_ineq}
%  \Prob_{\mathbf{U}}\left[|J(\mathbf{k}% _{\mathrm{mean}}
...
\begin{figure*}[!ht]
  ...
  \caption{Left: Contour of the $J_{\mathrm{BH}}$ function, conditional mean and variance. The estimates $\hatkmean$ and $\hatkvar$ are plotted with the dashed line. Right: $J_{\mathrm{BHswap}}$ function. The scale of the bottom plots have not been represented.} %TODO revoir
...
\section{A new family of robust estimators \elise{titre a revoir}}
...
$J^*(\mathbf{U})$ and 
$\alpha J^*(\mathbf{U})$. Without relaxation, i.e.\ when $\alpha$ is set to $1$, $\Gamma_1$ is non-zero if the set $\{\mathbf{u}\in\Uspace \mid J(\mathbf{k},\mathbf{u}) = J^*(\mathbf{u})\}$ has non-zero measure with respect to $\Prob_{\mathbf{U}}$. Still, this can be related to the definition of the MPE from Eq.~\eqref{eq:MPE}
...
\end{align}
that is the smallest $\alpha$, such that there exists a particular $\checkkp \in \Kspace$ for which $J(\checkkp,\mathbf{U}) \leq \checkap J^*(\mathbf{U})$ with probability $p$. This formulation can be linked to the Value-at-Risk of the random variable $\max_{\mathbf{k}} \{ \frac{J(\mathbf{k},\mathbf{U})}{J^*(\mathbf{U})}\}$, that is a measure of risk usually applied in the financial sector (see \cite{rockafellar_deviation_2002}). For different levels $p$, and thus different $\alpha_p$, Figure~\ref{fig:illu_alpha_p} shows examples of $\Gamma_{\alpha_p}$. The associated $\mathbf{k}_p$ is then the first value for which $\Gamma_{\alpha_p}$ reaches $p$.

\begin{figure}[!ht]
\centering
\includegraphics[width = 8.3cm]{Figures/illu_alpha_p.pdf}
\caption{Illustration of the influence of different levels $p$ on $\Gamma_{\alpha_p}$ and on $\mathbf{k}_p$ \victor{Est-ce que tu pensais à ce genre de figure ?}}
...
The smallest relaxation $\hat{\checka}_1$ and the corresponding $\hat{\checkk}_1$ has been computed for the $J_{\mathrm{BH}}$ and $J_{\mathrm{BHswap}}$ functions, using a regular grid of $1000 \times 1000$ points on $\Kspace\times\Uspace$. The contour plots of those functions can be seen in the top plots of Figure~\ref{fig:contour_alpha}. The frontier corresponding to the couples of points $(\mathbf{k},\mathbf{u})$ verifying $\{J(\mathbf{k},\mathbf{u}) = \hat{\checka}_1 J^*(\mathbf{u})\}$ has been drawn on top of these contour plots. 
On the bottom plots, the curve $\mathbf{k}\mapsto \hat{\Gamma}_{\alpha}(\mathbf{k})$ for $\alpha=\hat{\checka}_1$ and $\alpha=1.5$ along with the histogram of the minimisers have been represented.
...
\begin{figure}[!ht]
\includegraphics[width=12cm]{Figures/alpha_p_BH.pdf}
...
\begin{tabular}{lr}
\tophline
Estimate & Value \\ \middlehline
$\hatkvar$ & 1.371 \\ 
$\kest_p,~p=1$ & 1.557 \\ 
$\hatkmean$ & 1.587 \\ 
$\hatkmpe$ & 1.628 \\ 
$\kest_p,~p=0.95$ & 1.672 \\ 
$\kest_{\mathrm{maxratio}},~\hat{p}_{\mathrm{maxratio}}=0.654$ & 1.637 \\ \bottomhline
\end{tabular}
\end{table*}
...
\begin{figure*}[t]
\includegraphics[width=12cm]{Figures/profile_BH_all_estimates_noku.pdf}
...
\begin{figure}[ht]
\includegraphics[width=12cm]{Figures/alpha_p_BHs.pdf}
...
\begin{figure*}[t]
\includegraphics[width=12cm]{Figures/profile_BHs_all_estimates_noku.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[width=12cm]{Figures/RobustSWEmis_plots.pdf}
...
\end{equation}
We can then get an estimation of the mean squared forecast per volume per time step:

\begin{equation}
  \label{eq:squared_forecast_error_averaged}
  S(\mathbf{k}) = \frac{1}{N_{\mathbf{u}} N_{\mathbf{u}'}}\sum_{\mathbf{u},\mathbf{u}'} S_{\mathrm{pred}}(\mathbf{k},\mathbf{u},\mathbf{u}') = \frac{1}{N_{\mathbf{u}} N_{\mathbf{u}'}}\sum_{\mathbf{u},\mathbf{u}'}\left(\mathcal{M}_{\mathrm{pred}}(\mathbf{k},\mathbf{u}) - \mathcal{M}^{o}_{\mathrm{pred}}(\mathbf{u}')\right)^2
\end{equation}

\subsubsection{Averaged Quantile score}
A first approach is to compare the quantiles obtained by the prediction $\mathcal{M}_{\mathrm{pred}}(\kest,\mathbf{U})$, with the quantiles observed of $\mathcal{M}^o_{\mathrm{pred}}(\mathbf{U})$.
We are first going to introduce the pinball loss, or checkloss function:
\begin{equation}
\label{eq:checkloss}
