...
\end{equation}
In Figure~\ref{fig:relax_tuto} \victor{Mettre description}. 
...
\caption{Principle of the relaxation of the constraint on $J_{\mathrm{BHswap}}$, and illustration of $R_\alpha(\mathbf{k})$. Top plots: mieux décrire}
...
Noting that without relaxation, \textit{i.e.} when $\alpha$ is set to $1$, $\Gamma_1$ is non-zero if the set $\{\mathbf{u}\in\Uspace \mid J(\mathbf{k},\mathbf{u}) = J^*(\mathbf{u})\}$ has non-zero measure with respect to $\Prob_{\mathbf{U}}$. It happens when the distribution of $\mathbf{K}^*$ presents atoms \victor{?}. This can be linked to the definition of the MPE from Eq.~\eqref{eq:MPE}. In the more general case, the MPE can be seen as the limiting case when $\alpha$ tends to $1$.
...
$\checkap$ can be an indicator of the robustness of $\ckeckkp$.  
