...
Given the probabilistic nature of the environmental conditions $\mathbf{U}$, the forecasts will also be probabilistic. We will then compare $\mathcal{M}_{\mathrm{pred}}^o(\mathbf{U})$ and $\mathcal{M}_{\mathrm{pred}}(\mathbf{k},\mathbf{U})$, for a calibrated $\mathbf{k}$. The metric of comparison will be the squared forecast error, and the Continuous Ranked Probability Score (CRPS). The calibrated values that are compared are the minimiser of the expectation $\hatkmean$, the minimiser of the variance $\hatkvar$, the relative regret of level $p=1$: $\kest_1$ as described in the previous section, but also the global minimiser $\kest_{\mathrm{global}}$, and some conditional minimisers $\kest^*(\mathbf{u}^i)$ where the chosen environmental variables are $\{\mathbf{u}^i\}_{1\leq i \leq 4} = \{0.5, 0.55, 0.65, 0.7\}$. $\kest^*(0.6)$ has been omitted due to the fact that it result in a value very similar to the minimum of the expectation. These have been included in order to compare the different metrics obtained without using a robust approach. \todo{à revoir}
...
One issue with the squared forecast error is that it will penalize strongly a high sensitivity to the environmental conditions \todo{à conserver/changer ?}. A way to deal with this is to use another metric, that takes the probabilistic nature of the forecasts into account.
...
The calibrated parameters that present the best performances for forecasts according to those two metrics are $\kest^*(0.7)$, $\kest^*(0.65)$ which is very similar to $\kest_{\mathrm{global}}$.
