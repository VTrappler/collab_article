...
\begin{figure*}[!ht]
  \centering
  \includegraphics[width=10cm]{Figures/branin_side_moments_noscale.pdf}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=10cm]{Figures/branin_side_66.pdf}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=10cm]{Figures/relaxation_tuto.pdf}
...
\begin{figure}[!ht]
\centering
\includegraphics[width = 0.6\textwidth]{Figures/illu_alpha_p.pdf}
...
We have seen how some classical robust estimators and the \RRE{} behave on two different analytical problems. In addition to the usual levels of confidence such as 90\% and 95\%, one can also settle for a ad-hoc compromise, where $p$ maximizes the ratio $p/\alpha_p$. When the sample space of $\mathbf{U}$ is bounded, a conservative solution is to set $p=1$.
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.8\textwidth]{Figures/alpha_SWE}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=.75\textwidth]{Figures/profile_swe}
...
\begin{figure*}[!h]
\centering
\includegraphics[width=.8\textwidth]{Figures/L2_full_rel.pdf}
\caption{Squared forecast error, depending on the calibrated parameter. The left figure shows the squared error averaged over time, while the right barplot shows the relative change of the mean forecast error, averaged over time and space, taken w.r.t.\ to the best attained performance for $\mathbf{k}^*(0.7)$}
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=.8\textwidth]{Figures/CRPS_full_rel}
  \caption{CRPS computed for different calibrated parameters. The left figure show the CRPS averaged over time, and the right figure shows the relative change of the CRPS averaged over time and space, taken with respect to the best attained CRPS for $\mathbf{k}^*(0.7)$}
\label{fig:forecast_crps}
\end{figure}

The left plot of Figure~\ref{fig:forecast_crps} shows the CRPS averaged over time, and the right plot shows the value of the CRPS averaged over time and space. The difference between the squared forecast error and the CRPS is apparent when comparing the general trends shared by the different calibrated parameters. According to the squared error, the region around $x=4$ is not well predicted, while around $x=8$, the predictions are better. On the other hand, according to the CRPS, the region around $x=8$ provide worse forecasts than when $x=4$ and $x=7$. Given the properties of the two metrics,  we can conclude that the region around $x=4$ presents a lot of variability with respect to $\mathbf{u}$, for both the true model and the numerical one. However, for $x \approx 8$, there is a lot less variability, as the low squared error indicates, but probably a higher bias, due to the systematic errors between the truth and the numerical model.
...
For the calibration of the shallow water model presented in Section~\ref{sec:SWE_application}, we chose to construct a response surface based on Gaussian processes, which is going to be used for the extensive computations. This construction is first based on an initial design, that is enriched to better capture the locus of the conditional minimisers using the PEI criterion introduced in~\cite{ginsbourger_bayesian_2014}. One lead that appears naturally to diminish the amount of computation is to find a sequential criterion that makes the balance between the minimisation of a probability of failure, and 

Comparing the performances of the different estimates, one can see that for the two metrics introduced in Section~\ref{sec:SWE_application}, $\kest_1$ performs better than $\hatkmean$, but is sometimes outperformed by ``non-robust'' solutions.
