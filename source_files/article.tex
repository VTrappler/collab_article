...
\documentclass[preprint,review, 1p]{elsarticle}
...
\newcommand{\victor}[1]{{\itshape\color{blue}}}
\newcommand{\remove}[1]{{\color{red} {}}}
\newcommand{\added}[1]{{\color{teal} #1}}
\usepackage{url}
\usepackage{xcolor}
...
\author[1]{Victor {Trappler}\corref{cor1}}
...
\address[1]{Univ. Grenoble Alpes, Inria, CNRS, Grenoble INP\textsuperscript{1}\fnref{fn1}, LJK, 38000 Grenoble, France}
...
\begin{abstract}
%% Text of abstract
  Classical methods of \added{calibration}\remove{parameter estimation} usually imply the minimisation of an objective function \added{with respect to some control parameters. This function}\remove{, that} measures the error between some observations and the results obtained by a numerical model. In the presence of %uncontrollable/random nuisance
  \added{uncontrollable additional parameters that we model as} random inputs, the objective function becomes a random variable, and notions of robustness have to be introduced \added{for such an optimisation problem.}

  
  \remove{In this paper, we are going to present how to take into account those uncertainties by defining a family of calibration objectives based on the notion of relative-regret. with respect to the best attainable performance given the uncertainties and compare it with the minimum in the mean sense, and the minimum of variance.}


  \added{In this paper, we are going to present how to take into account those uncertainties by defining the relative-regret. This quantity allow us to compare the value of the objective function to its best performance achievable  given a realisation of the random additional parameters. By controlling this relative-regret using a probabilistic constraint, we can then define a new family of estimators, whose robustness with respect to the random inputs can be adjusted.}
...
%
An application of such an approach, in the context of ocean circulation modelling, is the estimation of ocean bottom friction parameters, as done in~\cite{das_estimation_1991,das_variational_1992,boutet_estimation_2015}. \added{Moreover, as such studies are often performed at a coastal and regional scale, those models are often designed with open boundary conditions, and require external forcings, such as tidal and wind forcing. Those are then a source of aleatoric uncertainties, that should be taken into account as in~\cite{ten_brummelhuis_parameter_1990}.}
\remove{If parameters to be estimated are not the only source of uncertainties, their optimal control 
...
\added{If we overlook the aleatoric uncertainties by choosing a specific outcome, the optimal control of the parameters to be estimated can lead to \emph{localized optimisation} \citep{huyse_free-form_2001} and \emph{overcalibration}, that is choosing a value that is optimal for the given situation. This value does not carry the optimality to other situations.
  In geophysics and especially in hydrological models, this overcalibration may lead to the appearance of abberations in the predictions as those uncertainties become prevalent sources of errors. In hydrology, uncertainties are the principal culprit of the existence of so called ``Hydrological monsters''~\citep{kuczera_there_2010}, that are calibrated models that perform really badly because some uncertainties in the modelling have been omitted, such as measurements errors. In flood modelling~\cite{wong_neglecting_2018}, the aleatoric uncertainty come from the structure of the model, and neglecting those uncertainties leads to underestimating hazard. In other domains as well, the aleatoric uncertainties can represent some manufacturing errors or environmental conditions such as wind direction and speed in wind turbine modelling~\cite{hu_robust_2011} or atmospheric conditions in aerospace vehicle design~\cite{brevault_contributions_2015}, thus they represent an important role in risk management.}
...
\added{It is then crucial to be able to take into account aleatoric uncertainties in optimisation problems. This consideration is called \textit{robust optimisation}, or also \textit{robust design} in~\cite{lelievre_consideration_2016}, or \textit{optimisation under uncertainties} \citep{petrone_robustness_2011,seshadri_density-matching_2014,cook_horsetail_2018}. Furthermore, the distinction is sometimes made between \emph{stochastic optimisation} and \emph{robust optimisation}, depending on the knowledge available on the aleatoric uncertainties.}% Présicer pbs + autres exemple avec unc aleatoric prises en compte -> flood
% \remove{Taking into account aleatoric uncertainties in optimisation problems takes several names, including \textit{robust optimisation}, \textit{robust design} in~\cite{lelievre_consideration_2016}, or \textit{optimisation under uncertainties} \citep{petrone_robustness_2011,seshadri_density-matching_2014,cook_horsetail_2018}.}

%In this paper we adopt the \textit{robust} terminology.

%Taking into account aleatoric uncertainties while calibrating the model (reducing epistemic uncertainties) is often referred as {\it robust control problem} in the literature (see for instance, \arthur{REFs}), or optimisation under uncertainties \citep{petrone_robustness_2011, seshadri_density-matching_2014,cook_horsetail_2018}.

\remove{Let's} \added{Let us} denote $\mathbf{k} \in \Kspace$ the parameter \remove{set} to be estimated \added{in order} to reduce epistemic \remove{errors} \added{uncertainties}. \added{We assume that the aleatoric uncertainties can be modelled as a} random vector $\mathbf{U}$ whose sample space is $\Uspace$. The probability measure \added{associated with}\remove{of} $\mathbf{U}$ is $\Prob_{\mathbf{U}}$, and its density, if it exists, is $p_{\mathbf{U}}$. \added{This distribution is assumed to be known, and that it is possible to sample from it. This choice is motivated by the fact that in various applications, the aleatoric uncertainties come from expert knowledge, empirical observations, or the knowledge acquired using other models, for instance by ensemble assimilation. %  An example is functional uncertainties, where the random process can be decomposed on an orthogonal basis of random variables, which are uncorrelated and can be estimated.
  However, if it is not the case,~\cite{ning_optimization_2019} provides a comprehensive review of optimisation under uncertainty, especially on the modelling and sampling of the aleatoric uncertainties. Since the source of the aleatoric uncertainties is considered external, the choice of $\mathbf{k}$ does not have any influence on the distribution of the random variable $\mathbf{U}$, and therefore the aleatoric and epistemic uncertainties are assumed independent.
  %%%%

}

\victor{\#3 \checkmark: Did the authors implicitly assume that the aleatoric uncertainty is independent with the epistemic uncertainty? The authors should clarify this issue.}%Furthermore, $\mathbf{u}^t$ (used to generate $\mathbf{y}^{\mathrm{obs}}$) is sampled from $\mathbf{U}$.
%
...
\victor{
  \begin{itemize}
  \item Control on $\mathbf{k}$ but not on $\mathbf{u}$ + examples \checkmark
  \item Misspecification and monsters \checkmark
  \item Challenging examples
  \item Give example of $J$, $\mathbf{k}$, $\mathbf{u}$
  \end{itemize}
}
\begin{linenomath*}
\begin{equation}
...
%$J$ is possibly complemented by a regularisation function that is omitted here for the sake of clarity. 
  For instance, in data assimilation, $J$ describes a distance between the output of the numerical model and given observed data, plus generally some regularization terms. \added{An example of such a function will be treated Section~\ref{sec:SWE_application}.}
...
The definition of robustness differs depending on the context in which it is used. Indeed, one definition of the robustness of an estimate is a measure of the sensibility of said estimate to outliers~\citep{huber_robust_2011}. This \remove{lead}\added{leads} to the introduction of robust norms in data assimilation \citep{rao_robust_2015}. %The point of this norm is to assume Gaussian distribution for small errors (the usual $L_2$ norm), while the large errors are assumed to be Laplace distributed (so the $L_1$ norm).
In a Bayesian framework, robustness may refer to the sensitivity to a wrong specification of the priors~\citep{berger_overview_1994}. \added{As the distribution of $\mathbf{U}$ is assumed known, we are not going to consider \emph{distributional robustness}, which deals with uncertainties on the distribution of $\mathbf{U}$~\cite{rahimian_distributionally_2019}.} 
...
The usual practice consists in neglecting the variability of $\mathbf{U}$ by setting it to an {\it a priori} value $\mathbf{u}^b$.
 In this case, $\kest$ is set to the optimum ${\mathbf{k}}_{\mathbf{u}^{b}} $ of $J(\mathbf{k}, \mathbf{U}=\mathbf{u}^b)$.  There is no guarantee on the performance of $\kest$ if the calibrated model is used for predictions, as the estimated value will compensate the error made by a possibly wrong specification of $\mathbf{u}^b$\added{: this may be a case of \emph{overcalibration}}
 In a data assimilation context, this situation appears if $\mathbf{u}^b$ does not properly represent the conditions \remove{on}\added{in} which the observations have been obtained.

 
Another strategy, that consists in minimising $J$ over the joint space $\Kspace\times\Uspace$, is not always possible or relevant. \added{Indeed, joint optimisation increases} the complexity of the optimisation\remove{is increased}, and the computed estimation of $\kest$ has no reason to be robust in the end: this kind of method does not take into consideration the \added{intrinsic} variability of the \added{environmental}\remove{uncertain} variable. 
%
...
the distribution of the conditional minimisers, notion that appeared notably in~\cite{villemonteix_informational_2006} and in~\cite{hennig_entropy_2011} for a global optimisation purpose. Both approaches and related robust estimates are described in Section~\ref{robust formulations}. Section~\ref{sec:relax_constraint} introduces a new class of estimators, by relaxing the constraint of optimality and defining regions of acceptability, similarly as~\cite{buhmann_robust_2013} in discrete combinatorial problems, or~\cite{kouvelis_algorithms_1992,snyder_stochastic_2004} in operation\added{s} research. \added{The rationale behind this relaxation is to be able to construct an estimate $\kest$ which produces values of the cost function close enough from the minimal cost attainable given the configuration induced by $\mathbf{u} \in \Uspace$, with high enough probability.}\remove{The intention is that a robust estimate provides values of the cost function close enough to the attainable minimum for each configuration induced by $\mathbf{u}\in\Uspace$.} \added{This similarity will be measured using the relative regret. By adjusting either the relaxation or the confidence level, we can then define the \RRE{}, the relative-regret family estimators.  }Illustration of the various described methods are given on a numerical \remove{exemple}\added{example} in Section~\ref{sec:SWE_application}.
...
\victor{\# 4: The literature review on optimization under uncertainties is relatively good. However, it still needs improvement to reflect the recent advances in this area. To help the reader get to know the state-of-the-art, the authors should add the work on "optimization under uncertainty in the era of big data and deep learning" in the revised manuscript;\checkmark}
...
As mentioned before, robustness can be understood as satisfactory for a broad range of $\mathbf{u}$, and/or as insensitive as possible to uncertainties encompassed in $\mathbf{U}$. Under this definition, one may design robust estimators \added{of $\mathbf{k}$}, either by using the moments of the cost function; or by exploiting the distribution of its minimisers.
% ----------------------------------------------------------------------------
...
In general, an analytical form of $p_{\mathbf{K}^*}$ is \remove{usually} impossible to obtain, so an estimation $\hat{p}_{\mathbf{K}^*}$ must be used, and its maximum computed to get the MPE.
...
The left\remove{,} (resp.\ right)\remove{,} column stands for $\JBH$ (resp.\ $\JBHS$).
...
The density of minimisers has been estimated by optimising $\mathbf{k}\mapsto J(\mathbf{k},\mathbf{U}=\mathbf{u})$ over $\Kspace$ for different realisations of $\mathbf{u}$. Instead of focusing on optimal values, we propose
\added{to construct an \textit{acceptable} {neighbourhood} in terms of performances of the cost function as well. In order to do so, we are going to introduce a relaxation coefficient $\alpha \geq 1$, and given $\mathbf{u}\in\Uspace$, we say that $\mathbf{k}\in \Kspace$ is acceptable when $J(\mathbf{k},\mathbf{U}=\mathbf{u}) \leq \alpha J^*(\mathbf{u})$. Using the strict positivity of the objective function, we can define the relative-regret as the ratio $J(\mathbf{k},\mathbf{U=u})/J^*(\mathbf{u})$, and the acceptability of $\mathbf{k}$ will then be tied to the maximal value taken by this ratio.}


\remove{to consider their {\it acceptable} neighbourhood in terms of performance of the cost function as well.
To do so, for each $\mathbf{u}$, $\mathbf{k}$ is deemed acceptable when  $J(\mathbf{k},\mathbf{U}=\mathbf{u}) \leq \alpha J^*(\mathbf{u})$, with $\alpha > 1$. \added{For $\mathbf{k}$, $\mathbf{u}$. The relative-regret is defined as the ratio $J(\mathbf{k},\mathbf{U=u})/J^*(\mathbf{u})$, for $\mathbf{u}\in \Uspace$, we say that $\mathbf{k}$ is acceptable at a level $\alpha$ if relative-regret is smaller than $\alpha$}}
%
...
This linked to the definition of the distribution of the minimisers $\mathbf{K}^*$. For instance, if $\Kspace$ is a discrete set, \added{$\mathbf{K}^*$ is a discrete random variable}, and we can rewrite $\Gamma_1$ as $\Gamma_1(\mathbf{k})=\Prob_{\mathbf{U}}\left[J(\mathbf{k},\mathbf{U}=\mathbf{u}) = J^*(\mathbf{U})\right] = \Prob_{\mathbf{U}}\left[\mathbf{k} = \mathbf{k}^*(\mathbf{U})\right]$ which is the probability mass function of $\mathbf{K}^*$.
...
\added{We consider in this work exclusively a multiplicative relaxation, instead of an additive one in the form $J(\mathbf{k},\mathbf{u}) \leq J^*(\mathbf{u}) + \beta$ for $\beta > 0$. Both formulations of the regret: the additive $J-J^*$ and the relative $J/J^*$, take into account the value of the conditional minimum $J^*(\mathbf{u})$, but relative regret allow us to scale the region of acceptable points with respect to the best situation available: if $J^*(\mathbf{u})$ is close to $0$, the region of acceptable points will grow slowly, so at a given $\alpha$, we put emphasis on $\mathbf{k}^*(\mathbf{u})$. On the other hand, if $J^*(\mathbf{u})$ is large, it means that tuning the parameter to $\mathbf{k}^*(\mathbf{u})$ will not lead to very good performances of the cost function anyway. We can then put less weight on this value $\mathbf{k}^*(\mathbf{u})$ in the estimation, hence the region of acceptability that grows quickly. Finally, the relative regret is a normalized quantity, in constrast to the additive regret, which shares the same unit as $J$. This normalization, as well as the direct relation to the relative error $\frac{J - J^*}{J^*}$ allows for an interpretation of the relaxation as a fraction of error.}
...
\added{Rewriting the equation above, we can express $\alpha_p$ as the solution of the following chance constrained problem}
% \added{\begin{equation}\label{eq:chance_constraint_formulation}
%   \left\{
%   \begin{array}{rl}
%     \min  & q \\
%     \text{such that}& \max_{\mathbf{k} \in \Kspace}\Prob_{\mathbf{U}}\left[\frac{J(\mathbf{k},\mathbf{U})}{J^*(\mathbf{U})} \leq q \right] \geq p
%   \end{array}
%   \right.
% \end{equation}
\added{
  \begin{linenomath*}
\begin{equation}\label{eq:chance_constraint_formulation}
  \left\{
  \begin{array}{rl}
    \min & q \\
    \text{s.t.} &% \max_{\mathbf{k}\in \Kspace}
                  \max_{\mathbf{k}}\Prob_{\mathbf{U}}\left[\frac{J(\mathbf{k},\mathbf{U})}{J^*(\mathbf{U})} \leq q \right] \geq p% \quad \forall \mathbf{k}
  \end{array}
  \right.
\end{equation}
\end{linenomath*}
...
\added{As highlighted by the formulation of Eq.~\eqref{eq:chance_constraint_formulation}, $\mathbf{k}_p$ and $\alpha_p$ are the result of the optimisation }of the Value-at-Risk of the random variable $J(\mathbf{k},\mathbf{U}) / J^*(\mathbf{U})$, \remove{that}\added{which} is a measure of risk usually applied in the financial sector (see~\cite{rockafellar_deviation_2002}).
\added{In this spirit, when choosing $\mathbf{k}_p$, the relative error of the function $J$ will be less than $\alpha_p$ with probability $p$. The maximal relative regret of the function will be $\alpha_p$, except for the $100(1-p)\%$ least favourable cases.}

\added{The set of maximisers of $\Gamma_{\alpha_p}$ for different $p$: $\{\mathbf{k}_p \text{ for } p \in [0;1] \}$ is what we are calling the relative-regret family of estimators \RRE.}
\remove{$\{\mathbf{k}_p \text{ for } p \in [0;1] \}$ defines the relative-regret family of robust estimators (\RRE).}


\remove{For different levels $p$, and thus different $\alpha_p$, }Figure~\ref{fig:illu_alpha_p} shows examples of $\Gamma_{\alpha_p}$ for $\JBHS$\added{, at different levels $p$, and the associated estimates of the \RRE.}
\remove{By definition, the associated $\mathbf{k}_p$ is then the first value for which $\Gamma_{\alpha_p}$ reaches $p$.} We can see that changing the level $p$ shifts the maximiser of $\Gamma_{\alpha_p}$\remove{, and that for small $\alpha$, $\added{\max} \Gamma_{\alpha}\remove{(\mathbf{k}_1)}$ is also very small}. \added{Since} \remove{This indicates that }$\mathbf{k}_1$ is located quite far from the conditional minimisers, it arises as a compromise when the relaxation is large enough.

\begin{figure}[!ht]
\centering
\includegraphics[width = \singlecolumnsize]{Figures/FIG04.pdf}
\caption{Illustration of the influence of different levels $p$ on $\Gamma_{\alpha_p}$ and on $\mathbf{k}_p$ for $\JBHS$. \added{When $p$ increases, $\Gamma_{\alpha_p}$ increases as well.} \victor{\# 7: difficult to identify the difference in color of the last two lines. The authors should revise the figure accordingly.\checkmark}
}
\label{fig:illu_alpha_p}
\end{figure}


\victor{changed place}
\remove{$\checkap$ can be \added{interpreted as} an indicator of the robustness of $\checkkp$.}%
\remove{A high value of $\checka_1$ corresponds to a high ratio ${J(\mathbf{k},\mathbf{U}=\mathbf{u})/J^*(\mathbf{u})}$ for at least one particular $\mathbf{u}$. In that sense, $J(\checkk_1,\mathbf{U}=\mathbf{u})$ may be possibly very high.}%
\remove{More generally, for a fixed level of confidence $p$, $\checkap$ is the slackness needed to be able to reach the probability $p$. % So, in addition to the value of the estimate $\checkkp$, we have an indicator of the robustness associated.
We can then see that it is sensible to study jointly $p$, $\checkap$ and $\checkkp$.}
\added{We have then introduced the \RRE, a family of estimators among which we can choose either large level of confidence $p$ \textit{i.e.} a large robustness as we are controlling the relative regret for a large fraction of possible configurations, or we can look for almost optimal performances albeit for a more reduced number of situations. Those quantities require the evaluation and optimisation of probabilities, so solving such a problem can be quite challenging. Moreover, the choice of a member of the \RRE, that is the choice of a level of confidence $p$ is also up to the modeller.}
...
\subsection{\remove{Choosing the relaxation coefficient}\added{Choice and computation of the relaxation coefficient}}%Finding a balance between $p$ and $\checkap$}%Selection of a level of confidence}
\label{ssec:balance}
\added{\subsubsection{Almost-surely bounded relative-regret}}


\added{Let us first consider that we want to satisfy the chance constraint of Eq.~\eqref{eq:chance_constraint_formulation} almost surely: this is the particular case}
\remove{By considering the particular case}\remove{where}\added{of} $p=1$.
\added{Given the strict positivity of $J$, the choice of such a level of confidence is possible only if there exists a $\mathbf{k}$ such that the ratio $J(\mathbf{k},\mathbf{U})/J^*(\mathbf{U})$ is bounded almost surely, and}
\added{
$\alpha_1$ is the solution of the (almost-surely) chance constrained problem:
\begin{linenomath*}
  \begin{equation}
    \label{eq:chance_as}
    \left\{
    \begin{array}{rl}
      \min & q \\
      \text{s.t.} & % \max_{\mathbf{k} \in \Kspace}
                    \min_{\mathbf{k}}\frac{J(\mathbf{k}, \mathbf{U})}{J^*(\mathbf{U})} \leq q \quad % \forall \mathbf{k} \in \Kspace 
                    \text{ a.s.}
    \end{array} \right.
\end{equation}
\end{linenomath*}
...
\remove{ and }\added{thus, $\mathbf{k}_1$ verifies}
\begin{linenomath*}
\begin{equation}
  \label{eq:def_alpha1} % This is eq19 originally
  \Prob_{\mathbf{U}}\left[\frac{J(\mathbf{k}_1,\mathbf{U})}{ J^*(\mathbf{U}) } \leq \checka_1\right]=1 \added{\iff \frac{J(\mathbf{k}_1,\mathbf{U})}{J^*(\mathbf{U})} \leq \alpha_1 \quad \text{a.s.}}
\end{equation}
\end{linenomath*}
...
This alternative definition of $\checka_1$ is useful to estimate it quickly whenever a grid over $\mathbb{K}\times\mathbb{U}$ has already been evaluated, thus avoiding multiple computations of $\Gamma_{\alpha}(\mathbf{k})$ until its maximum reaches $1$. From Eq.~{\eqref{eq:upper_bound_alpha_check}}, choosing a level of confidence equal to $1$ is then equivalent to looking for the worst-case scenario of the ratio ${J(\mathbf{k},\mathbf{U}=\mathbf{u})/J^*(\mathbf{u})}$. }



 \added{Under the assumption that $J(\mathbf{k},\cdot)$ is continuous for all $\mathbf{k}$, this is equivalent to approaching this problem using uncertain sets to model the uncertain nature of $\mathbf{U}$:
   \begin{linenomath*}
  \begin{equation}
    \frac{J(\mathbf{k}_1,\mathbf{u})}{J^*(\mathbf{u})} \leq \alpha_1 \quad \forall \mathbf{u} \in \Uspace
  \end{equation}
\end{linenomath*}
...
\begin{equation}
  \label{eq:upper_bound_alpha_check}
  \checka_1 = \max_{\mathbf{u}\in\Uspace} \frac{J(\mathbf{k}_1,\mathbf{U}=\mathbf{u})}{J^*(\mathbf{u})} =  \min_{\mathbf{k}\in\Kspace} \left\{ \max_{\mathbf{u}\in\Uspace} \frac{J(\mathbf{k},\mathbf{U}=\mathbf{u})}{J^*(\mathbf{u})} \right\}
\end{equation}
\end{linenomath*}

}

\added{This can then be linked to Savage's minimax approach~\cite{savage_theory_1951}, which consists in a worst-case approach for the additive regret $J - J^*$, when in Eq.~\eqref{eq:upper_bound_alpha_check} we look to minimise the worst-case scenario in terms of the ratio.}




\remove{An illustration of this flaw appears in the following simple problem:
  \begin{linenomath*}
\begin{align*}
  \label{eq:Jinfinite}
...
% \remove{a relation between $\checkk_1$ and $\checka_1$ can be derived, by using Eq.~\eqref{eq:def_alpha_check} and the strict positivity of the cost function:}
...
\victor{\# 8:  For Equation (19), if the probability distribution Pu is perfectly known for the chance constraint, it seems to be trivial to the sample average approximation (SAA) method. How can the authors quantify the error of using this approximation method? Will it have an impact on the calibration results? The authors should explain it in detail.}

\added{
 Using Sample Average Approximation (SAA), based on $N$ i.i.d.\ samples of $\mathbf{U}$: $\{\mathbf{u}^i\}_{1 \leq i \leq N}$, we can reformulate the problem Eq.~\eqref{eq:chance_as} as
 \begin{linenomath*}
  \begin{equation}
    \label{eq:SAA_approx_as}
    \left\{
    \begin{array}{rl}
      \min & q \\
      \text{s.t.} & \min_{\mathbf{k}}\frac{J(\mathbf{k}, \mathbf{u^i})}{J^*(\mathbf{u^i})} \leq q \quad \text{ for }1\leq i \leq N 
    \end{array} \right.
  \end{equation}
\end{linenomath*}

  For a solution $\hat{\alpha}_1$ of Eq.~\eqref{eq:SAA_approx_as}, as SAA acts as a relaxation of the initial problem Eq.~\eqref{eq:chance_as}, the solution found acts as lower bound on the true value $\alpha_1$: $\hat{\alpha}_1 \leq \alpha_1$.
  
  Moreover, this estimated value $\hat{\alpha}_1$ can be used for the estimation of the relaxation constant for a level $p$ close to $1$. Using Clopper-Pearson intervals~\cite{luedtke_sample_2008,thulin_cost_2014}, if $\hat{\alpha}_1$ is a solution of Eq.~\eqref{eq:SAA_approx_as}, it is also a feasible solution of Eq.~\eqref{eq:chance_constraint_formulation} at a level $p=\left(\frac{\eta}{2}\right)^{1/N}$ with probability $1-\eta$: $\hat{\alpha}_1$ is then a probabilistic upper bound on $\alpha_{\left(\frac{\eta}{2}\right)^\frac{1}{N}}$.
  \begin{linenomath*}
\begin{equation}
  \label{eq:CPinterval_alpha1}
  \hat{\alpha}_1 \in \left[\alpha_{\left(\frac{\eta}{2}\right)^{1/N}},  \alpha_1 \right]\text{ with probability }1-\eta
\end{equation}
\end{linenomath*}

 
  
  % $\Prob_{\mathbf{U}}\left[J(\hat{\mathbf{k}}_1,\mathbf{u}) \leq J^*(\mathbf{u})\right] \geq \left(\eta /2 \right)^{1/N}$ with probability $\eta$, so
  % $\Prob_{\mathbf{U}}\left[\left(\eta /2 \right)^{1/N} \leq \hat{\alpha}_1\right] = \eta$. $\hat{\alpha}_1 \in [\left(\eta /2 \right)^{1/N}, 1]$ with probability $\eta$.
  % \begin{align}
  %   \frac{1}{N}\log(\eta) -\frac{\log(2)}{N} \leq \log \hat{\alpha}_1 \\
  %   N \geq \frac{\log \eta - \log 2}{\log \hat{\alpha}_1}
  % \end{align}
The estimated value $\hat{\alpha}_1$ will then be optimistic in the sense that it will underestimate the true value $\alpha_1$. We can however control this approximation using the probabilistic bound lower bound of Eq.~\eqref{eq:CPinterval_alpha1}}

\added{Choosing a level of confidence $p=1$}\remove{Therefore it may} suffers from the same pitfall as the worst-case approach,\remove{As mentioned in the introduction, this is not suited for random variable with unbounded support} as it may return over-conservative solutions, provided that a solution exists in the first place.
...
\added{\subsubsection{Estimation of \RRE}}
\added{We are now going to focus on the more general case, where we can choose: to fix $p \leq 1$ and deduce $\alpha_p$, or to set a maximal threshold $\alpha$ and maximise the measure of the acceptable region, or finally to find a compromise $p$ and $\alpha_p$ in order to keep the latter not too large.}
\remove{In order to choose the quantities introduced above, there are three choices: either fixing $p$, either fixing the maximal threshold $\alpha$, or looking for a compromise between $p$ and $\alpha$.}



\added{A first approach}\remove{The most trivial way} is to set $\alpha$, \added{and then to estimate the function $\mathbf{k}\mapsto\Gamma_{\alpha}(\mathbf{k})$ % = \Prob_{\mathbf{U}}\left[J(\mathbf{k},\mathbf{U}) \leq \alpha J^*(\mathbf{U})\right]$
  as} \remove{\added{and then }to find the couples of points $(\mathbf{k},\mathbf{u})$ verifying $J(\mathbf{k},\mathbf{U}=\mathbf{u}) \leq \alpha J^*(\mathbf{u})$, and \added{use these} to estimate the probability $\Gamma_{\alpha}(\mathbf{k})$} defined in Eq.~\eqref{eq:def_Ralpha}.
For instance, when $N$ samples from $\mathbf{U}$ are available, namely $\{\mathbf{u}^i\}_{1\leq i \leq N}$, a possible estimate of $\Gamma_{\alpha}$ is
\begin{linenomath*}
\begin{equation}
\hat{\Gamma}_{\alpha}(\mathbf{k}) = \frac{\mathrm{Card} \left\{\mathbf{u}^i \mid J(\mathbf{k},\mathbf{U}=\mathbf{u}^i) \leq \alpha J^*(\mathbf{u}^i)\right\}}{N} \added{ = \frac{1}{N} \sum_{i=1}^N \mathbbm{1}_{\{J(\mathbf{k},\mathbf{U}=\mathbf{u}^i) \leq \alpha J^*(\mathbf{u}^i)\}}}
\end{equation}
\end{linenomath*}

\added{This expression is maximised with respect to $\mathbf{k}$, giving $\hat{p}=\max_{\mathbf{k}}\hat{\Gamma}_{\alpha}(\mathbf{k})$. We can also provide a confidence interval for the true value of $\Gamma_{\alpha}(\textbf{k})$ where $\textbf{k} = \argmax \hat{\Gamma}_{\alpha}$.}


\added{However, unless the modeller has some precise idea of desired relaxation, doing so may lead to an unsatisfactory pairing of $\alpha$ and $p$.}\remove{However, this may be a risky approach.} Indeed if $\alpha$ is chosen too small, the resulting $\hat{p}=\max_{\mathbf{k}} \hat{\Gamma}_{\alpha}(\mathbf{k})$ will be also small, meaning that the cost function will have non acceptable values with high probability.

Similarly, if $p$ is fixed, the corresponding $\hat{\checka}_p$ is computed by searching for the smallest $\alpha$ satisfying $\max_{\mathbf{k}} \hat{\Gamma}_{\alpha}(\mathbf{k})=p$\added{, or equivalently, by minimizing the quantile of order $p$ of the relative-regret, which is the ratio $J/J^*$}. Once again, if $\hat{\checka}_p$ is too large, \added{the cost function may not be controlled enough for the contemplated application.}\remove{ the relaxation needed to get acceptable values with probability $p$ is very high, so the resulting estimation $\hat{\checkk}_p$ may not be relevant for the future application.}

% \victor{chapeau car ici je parle de l'estimation en pratique}

% \added{
%   \begin{equation}\label{eq:SAAchance_constraint}
%   \left\{
%   \begin{array}{rl}
%     \min_{\mathbf{k} \in \Kspace} & q \\
%     \text{such that} &\frac{J(\mathbf{k},\mathbf{u}_i)}{J^*(\mathbf{u}_i)}  - q \leq M_i z_i \\
%                                   & \sum_{i=1}^N z_i \leq \gamma N \\
%                                   &z_i \in \{0, 1\}  \\
%                                   & M_i \geq \max_{\mathbf{k}} J(\mathbf{k},\mathbf{u}_i)/J^*(\mathbf{u}_i)
%   \end{array}
%   \right.
% \end{equation}}
...
We have seen how some classical robust estimators and the \RRE{} behave on two different analytical problems. In addition to the usual levels of confidence such as 90\% and 95\%, one can also settle for an ad-hoc compromise, where $p$ maximises the ratio $p/\alpha_p$. When the sample space of $\mathbf{U}$ is bounded, a conservative solution is to set $p=1$.
...
\added{Our approach is based on the comparison of the objective function to the optimal value it can attain, given a realisation of the uncertain variable. By constraining this ratio in terms of probability, the modeller can then minimize the frequency at which the relative error of the objective function will exceed a prescribed threshold. Alternatively, at a level of confidence $p$, the associated threshold $\alpha_p$ bounds the relative deviation of the objective function for the proportion $p$ of the most favourable cases. The resulting estimate can then be assessed robust, as it corresponds to values of the objective function as close as possible to the optimal values.}


\remove{Our approach is based on the will to be as close as possible to the conditional minimisers with high probability. At a level $p$, in addition to the estimation, we have the relaxation coefficient $\alpha_p$ that bounds with probability $p$ the relative error of the cost function. This helps us to assess the robustness properties of the proposed estimation.}
...
\remove{Comparing the performances of the different estimates in a forecast context, one can see that for the two metrics introduced in Section~\ref{sec:SWE_application}, $\kest_1$ performs better than $\hatkmean$, but is sometimes outperformed by ``non-robust'' solutions. Those require some additional knowledge about the environmental conditions, and did not present some robust properties during the calibration phase.}

Practically speaking, the estimation of those quantities is very crude, so the computational cost may be very expensive. As perspective, specific strategies have to be created in order to \added{carefully select each additional point that will be evaluated, and thus reduce} the amount of \added{evaluations}\remove{runs} needed of the numerical model. \added{This selection can be} based for instance on sequential design of computer experiments using Gaussian processes\added{~\cite{ginsbourger_towards_2010}}.  \remove{Ideally, one would want to develop an acquisition criterion, that makes the balance between the minimisation of a well-chosen probability of failure, and the exploration around the conditional minimisers.}
